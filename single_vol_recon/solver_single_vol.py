...
from single_vol_recon.model_single_vol_svr import  ReconNet, ReconNet_canonial_grid
from full_motion_simulations.full_simulation import SingleVolSVR_full_simulation
import datetime
...
class SVR_solver():
    def __init__(self, 
        ...
        self.rigid_trans_for_recon = torch.load(affine_matrix_path)[vol_to_opt::13,...].to(device)
        ...
        if full_sim_ds:
            self.dataset = SingleVolSVR_full_simulation(dwi_nii=os.path.join(path_to_ds,'scan1/ep2d_diff_64dir_iso1.6_s2p2_new_8.nii'), 
                                                        ...
                                                        voxel_size=rigid_config['voxel_size'],
            ...
            self.dataset = SingleVolSVR(experiment_path=path_to_ds,
                                        ...
                                        vol_indcies_for_case = vol_indices,
    ...
    def forward_trans_grid(self, data, iter=-1):
        ...
        # if iter%200 == 0:
        #     save_tensor_as_nii(recon_volumes[3].view(128,128,-1), f'debug_plots/training_recon_iter{iter}.nii.gz')
        ...
        #     g = F.affine_grid(inv_rigid_trans[i,:3,:].unsqueeze(0), size=(1,1,D,H,W), align_corners=True)  # [1,D,H,W,3]
        #     warped = F.grid_sample(recon_vol, g, mode='bilinear', padding_mode='zeros', align_corners=True)  # [1,1,D,H,W]
        #     trans_recons.append(warped)
        # trans_recons = torch.stack(trans_recons, dim=0)     # [N,D,H,W]
        # consensus_mean = trans_recons.mean(dim=0, keepdim=False) # [D,H,W]
        # consistency_loss = (trans_recons - consensus_mean).pow(2).mean()
